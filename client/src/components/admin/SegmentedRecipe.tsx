...
type Props = {
    recipe: { ingredient: Ingredient; amount: number, component: string }[];
    recipeComponents: { name: string, multiplier: number }[];
}

export default function SegmentedRecipe({ recipe, recipeComponents }: Props) {
    ...
    function getComponentIngredients(componentName: string) {
    ...
    return (
        <div className="max-w-5xl mx-auto mt-10 bg-white rounded-2xl shadow-md p-8">
          ...
          {segments.map((component) => {
            const ingredientsInComponent = getComponentIngredients(component.name);
